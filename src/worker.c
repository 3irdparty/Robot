...
static bool read_data(Worker *worker, Sock *sock)
{
    ...
    while(1)
    {
        ...
        while(nsave < nread)
        {
            ...
            if(!sock->msg)
            {
                if(left >= (PACK_HEAD_LEN - sock->len_readed))
                {
                    lencpy =(PACK_HEAD_LEN - sock->len_readed);
                    memcpy(sock->head+sock->len_readed, worker->buf + nsave, lencpy);
                    nsave += lencpy;
                ...
                {
                    memcpy(sock->head+sock->len_readed, worker->buf + nsave, left);
                    sock->len_readed += left;
                    nsave += left;
                    //nsave = nread;
                    goto Ok;
                }
            ...
            lencpy = (left >= (sock->len_total - sock->len_readed)) ? (sock->len_total - sock->len_readed) : left;
            memcpy(sock->msg->buf + sock->len_readed, worker->buf + nsave, lencpy);
            sock->len_readed += lencpy;
            nsave += lencpy;
            if(sock->len_readed == sock->len_total)
            {
                //printf("222 len: %d   save/read: %d / %d\n", sock->len_readed, nsave, nread);
                bool ret = handle_msg(sock->msg);
                //printf("333 len: %d   save/read: %d / %d\n", sock->len_readed, nsave, nread);
                sock->msg = 0;
                sock->len_readed = 0;
                //处理过程中出错，关闭清理sock
                if(!ret)
